export const GOOGLE_ERROR_MESSAGES = {
  ...
  google_cancelled: ['Google sign-in was cancelled.', 'Google साइन-इन रद्द झाले.'],
  google_no_code: [
  ...
  google_bad_state: [
    'Google login session was rejected. Try again in one tab.',
    'Google सत्र नाकारले.',
  ],
};

export function getGoogleErrorMessage(errorCode, language = 'en') {
  const pair = GOOGLE_ERROR_MESSAGES[errorCode];
  if (!pair) {
    return language === 'en'
      ? 'Unexpected login error. Please try again.'
      : 'अनपेक्षित लॉगिन त्रुटी.';
  }
